...
//    finalValue[x] = spectrum[x+10];
  ...
  lsm.read();
  float accel = lsm.accelData.x + lsm.accelData.y;
  if (accel > maxAccel) maxAccel = accel;
  if (accel < minAccel) minAccel = accel;
  float accelStep = (maxAccel - minAccel) / LEDS;
  float currentAccel = minAccel;
  for(x=0; x<LEDS; x++) {
//    strip.setPixelColor(x, color(finalValue[x*2], finalValue[x*2+1], 0));
    byte value = 255;
    if (currentAccel + accelStep > accel) {
      float delta = (accel - currentAccel) / accelStep;
      if (delta > 0.0)
        value = (byte)delta;
      else 
        value = 0;
    }
    strip.setPixelColor(x, color(value, value, value));
